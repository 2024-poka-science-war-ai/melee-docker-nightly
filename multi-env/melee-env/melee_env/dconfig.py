...
class DolphinConfig:
    def __init__(self):
        ...
        self.slippi_gecko_ini_path = (
            self.slippi_bin_path / "Sys/GameSettings/GALE01r2.ini"
        )
        self.install_data_path = self.module_path / "install_data"
        ...
        if not self.slippi_bin_path.exists():
            ...
            print(
                f"Successfully downloaded, installed, and configured dolphin "
    ...
    def use_render_interface(self, interface="opengl"):
        ...
        if interface not in ["vulkan", "opengl"]:
            raise ValueError(
                "unsupported render interface, please select "
    ...
    def set_controller_type(self, port, controller_type):
        ...
        if controller_type not in [e for e in melee.enums.ControllerType]:
            raise ValueError(
                f"Controller type must be one of {{{[e.name+':'+e.value for e in melee.enums.ControllerType]}}}"
            )
    ...
    def install_slippi(self, install_path):
        ...
        if self.platform == "win32":
            ...
            time.sleep(2)
            print(
                "Please make a decision to allow slippi-dolphin access to private/public networks..."
            )
    ...
    def apply_gecko_codes(self, install_path):
        ...
        self.slippi_gecko_ini_path.rename(
            self.slippi_gecko_ini_path.parents[0] / "GALE01r2.ini.old"
        )
        ...
        if data[14] != "\n":
            raise FileNotFoundError(
                "Something has gone wrong... check that {self.slippi_gecko_ini_path} exists."
            )
    ...
    def configure_dolphin(self, install_path):
        if not self.config_path.exists():
            raise FileNotFoundError(
                f"No Slippi Config. Run Slippi Online once to generate these files"
